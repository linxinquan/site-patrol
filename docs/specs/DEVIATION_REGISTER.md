...
| 批次 | 定义 | 条数 | 需你决策 | 建议节奏 |
|---|---|---|---|---|
| **批 0** | 红线/对外可信度问题 | 1 | 已定（选 B） | ✅ **已修**（2026-09-18）：文案改「已提交 / 待复核」 |
| **批 1** | 零决策：纯 bug / 死代码 / 过期注释 / 写死值 | 14 | 0 | ✅ **已修 13 条**（2026-09-18）；DV-03 按指示跳过 |
| **批 2** | 产品语义缺失与口径不统一 | 14 | 已定 | ✅ **已实施 10 条 / 消解 1 条 / 3 条转未来功能**，见 §3.1 |
...
**批 0 + 批 2 同日实施完毕**：13 项决策已全部落地或转未来功能，结果与存储设计见 §3.1。
...
### 3.1 批 0 + 批 2 实施结果（2026-09-18）

| 编号 | 你的决策 | 实施结果 |
|---|---|---|
| **DV-01**（批 0） | B | ✅ 胶囊改「**已提交 / 待复核**」，不再宣称校验；相关注释同步。真校验转 F1 |
| **DV-16** | B1 | ✅ 保存**不再建聚合缺陷**（`_saveRecordToStorage` 净删 75 行）；转入时逐条生成、带照片；**新增「按描述转入」**覆盖"AI 未识别但手写了描述"的情况 |
| **DV-17** | — | ✅ **自动消解**：只剩逐条一套口径，原「聚合条 vs 转入条字段分裂」随 DV-16-B1 消失 |
| **DV-18** | A | ✅ 设计师卡新增「**开始整改**」→`doing`、「**拒绝整改**」→`reject`（拒绝必填理由）；动作区改每行 3 个自适应换行 |
| **DV-19** | A | ✅ 新增 `Defect.sourceCaptureIdx`（转入时写入）；验收记录详情的「已转入」徽标现在显示**当前状态**（`已转入 · 整改中 / 已销项 / 已拒绝`）。存储设计见下方 |
| **DV-20** | 人工选 | ✅ 详情页新增「人工修订 → 专业分类」7 类选择器 |
| **DV-21** | ①②⑤ | ✅ 已做：①水印卡接**真实照片**（读 `photoPath`）②**备注块**（问题描述）⑤**未闭合说明**入口。③回复照片、④「待复核」状态位 → 转 F6 / F5 |
| **DV-22** | 字段清算 | ✅ 删 `photoHash` / `watermarkSerial` / `DefectStatusX.color+.soft` / `DefectSeverityX.soft`；扩展 `copyWith` 支持人工编辑；**另修一个 P0**：`toJson` 漏写 `designer*` 四字段导致重启后设计师处置结果丢失 |
| **DV-23** | B（记未来） | ⏸ 转 F3 |
| **DV-24** | B | ✅ 导出弹窗明示「按周期导出全部状态（含已销项），不受列表状态筛选影响」 |
| **DV-25** | B | ✅ 验收记录页「待整改」→「**待转入**」 |
| **DV-26** | 记未来 | ⏸ 转 F2。核实：`_location.name` 实为「附近定位点名称」（默认 `腾讯大铲湾 DY04 · 7栋`），**不是「已选点」**；问题是它随定位点切换而变，语义是"地点"而非"项目" |
| **DV-27** | B | ✅ 详情页「人工修订」可改**严重程度**与**问题描述** |
| **DV-28** | 不管 | ⏸ 登记为 F8 |
| **DV-29** | 归量尺 | ⏸ 登记为 F7 |

**DV-19 的存储设计（顺带回答"数据库怎么存"）**

- **前端**：`Defect.sourceCaptureId` + `sourceCaptureIdx` 成对；验收记录详情按二者反查缺陷状态，**不把状态冗余写回验收记录**（避免双写不一致）。
- **后端**：`captures` 存验收记录（含 `ai_result jsonb`）；`defects` 加 **`source_capture_id` + `source_capture_index`** 两列，建索引 `defects(source_capture_id)`；回流 = 查询时 join，**权威状态只有 Defect 一处**。
- **约定**：`sourceCaptureIdx = -1` 表示"按描述转入"（没有对应 AI 条目）。

**验证**：`dart analyze lib` → **0 error / 0 warning**。
...
| 2026-09-18 | **批 0 + 批 2 实施**（13 项决策）：DV-01 文案去伪校验、DV-16 改一事一录（保存不建缺陷）+ 按描述转入、DV-18 补 doing/reject 写入路径、DV-19 状态回流（含后端存储设计）、DV-20/27 详情页人工修订、DV-21①②⑤、DV-22 字段清算 + 修 P0 序列化缺失、DV-24/25 口径明示；3 条转 `FUTURE_BACKLOG.md` |
