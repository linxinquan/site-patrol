...
/// Web 预览的**吸附演示**：素材是取景井字格（横竖各两条）+ 画面中心。
///
/// 真实的点/线/物体边界吸附由原生 ARKit raycast + 平面边界求最近点完成
/// （见 `ArMeasureView.swift`）；Web 上拿不到 AR 场景，这里用取景辅助线
/// 复现**同一套交互与提示**（靠近即吸附、显示"已吸附：…"），
/// 保证浏览器里能验收吸附的观感与文案。
///
/// 返回吸附后的点与吸附类型描述；未命中任何辅助线时 [kind] 为 null（不吸附）。
({Offset point, String? kind}) snapToPreviewGuides(
  Offset p,
  Size size, {
  double tol = 26,
}) {
  final xs = [size.width / 3, size.width / 2, size.width * 2 / 3];
  final ys = [size.height / 3, size.height / 2, size.height * 2 / 3];
  double bestDx = tol;
  double? hitX;
  for (final x in xs) {
    final d = (x - p.dx).abs();
  ...
  if (hitX == null && hitY == null) return (point: p, kind: null);
  final onX = hitX != null;
  final onY = hitY != null;
  final q = Offset(hitX ?? p.dx, hitY ?? p.dy);
  final String kind;
  if (onX && onY) {
    kind = (hitX == size.width / 2 && hitY == size.height / 2)
        ? '中心交点'
        : '网格交点';
  } else if (onX) {
    kind = '竖网格线';
